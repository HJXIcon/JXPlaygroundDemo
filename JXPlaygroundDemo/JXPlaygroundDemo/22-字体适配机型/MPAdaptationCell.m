...
+(CGFloat)cellHegith:(NSString *)text
{
    ...
    if (text.length>0) {
        result=result+[[self cellTextAttributed:text] boundingRectWithSize:CGSizeMake(kScreenW-2*KLeftSpace, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin context:nil].size.height;
    }
    return result;
}
